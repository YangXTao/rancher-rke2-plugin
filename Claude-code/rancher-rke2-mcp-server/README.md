...
Every registered tool carries MCP `annotations`. Read-only tools such as
`get_capabilities`, `validate_config`, `build_plan`, `get_run`, and
`collect_diagnostics` are
...
clients such as Claude Code distinguish read-only calls from
