...
install_apt_dependencies() {
  ...
  if [[ -n "${HTTP_PROXY:-}" ]]; then
    ...
    proxy_opts+=("-o" "Acquire::https::Proxy=$HTTP_PROXY")
  fi
  apt-get "${proxy_opts[@]}" update
  protect_readonly_localtime
  if apt-get "${proxy_opts[@]}" install -y --no-upgrade ca-certificates curl unzip python3 iputils-ping; then
    protect_readonly_localtime
    return 0
  fi
  ...
  if grep -Fq ' /etc/localtime ' /proc/self/mountinfo &&
    ...
    mv -f "$backup" "$postinst"
    protect_readonly_localtime
    apt-get "${proxy_opts[@]}" install -f -y --no-upgrade
    return 0
  fi
  return 1
}
